...
const AddBook = ({refreshBooks}) => {
    ...
    return (
        <div>
            ...
            <button className="btn" onClick={()=>setModalOpen(true)}>Add Qoute</button>
            <dialog id="my_modal_3" className={`modal ${modalOpen ? "modal-open" : ""}`}>
                <form method="dialog" className="modal-box" onSubmit={handleSubmitNewBook}>
                    ...
                    <h3 className="font-bold text-lg">New Qoute</h3>
                    <input type="text" 
                        value={newBookTitle} 
                        onChange={e => setNewBookTitle(e.target.value)} 
                        placeholder="Enter New Qoute" 
                        className="input input-bordered w-full max-w-xs" />
                    <button type="submit" className='btn btn-primary'>Add Qoute</button>
                </form>
            </dialog>
        </div>
    )
